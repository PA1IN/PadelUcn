...
let CanchaService = class CanchaService {
    ...
    async create(createCanchaDto) {
        ...
        if (existingCancha) {
            throw new common_1.ConflictException(`Ya existe una cancha con el número ${createCanchaDto.numero}`);
        }
    ...
    async findByNumero(numero) {
        ...
        if (!cancha) {
            throw new common_1.NotFoundException(`Cancha con número ${numero} no encontrada`);
        }
        return cancha;
    }
