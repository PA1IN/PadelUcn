    ...
    async verificarDisponibilidad(canchaId, fecha, horaInicio, horaTermino, reservaIdExcluir) {
        try {
            ...
            for (const reserva of reservasExistentes) {
                const horaInicioStr = typeof reserva.horaInicio === 'string' ?
                    reserva.horaInicio :
                    reserva.horaInicio instanceof Date ?
                        reserva.horaInicio.toTimeString().split(' ')[0] :
                        '00:00:00';
                const horaTerminoStr = typeof reserva.horaTermino === 'string' ?
                    reserva.horaTermino :
                    reserva.horaTermino instanceof Date ?
                        reserva.horaTermino.toTimeString().split(' ')[0] :
                        '00:00:00';
                const horaInicioNuevoStr = typeof horaInicio === 'string' ?
                    horaInicio :
                    horaInicio instanceof Date ?
                        horaInicio.toTimeString().split(' ')[0] :
                        '00:00:00';
                const horaTerminoNuevoStr = typeof horaTermino === 'string' ?
                    horaTermino :
                    horaTermino instanceof Date ?
                        horaTermino.toTimeString().split(' ')[0] :
                        '00:00:00';
    ...
    async obtenerHorariosDisponibles(canchaId, fecha) {
        try {
            ...
            for (let hora = horaInicio; hora < horaFin; hora++) {
                ...
                for (const reserva of reservasExistentes) {
                    const inicioExistenteStr = typeof reserva.horaInicio === 'string' ?
                        reserva.horaInicio :
                        reserva.horaInicio instanceof Date ?
                            reserva.horaInicio.toTimeString().split(' ')[0] :
                            '00:00:00';
                    const terminoExistenteStr = typeof reserva.horaTermino === 'string' ?
                        reserva.horaTermino :
                        reserva.horaTermino instanceof Date ?
                            reserva.horaTermino.toTimeString().split(' ')[0] :
                            '00:00:00';
...
//# sourceMappingURL=reserva.service.fixed.js.map