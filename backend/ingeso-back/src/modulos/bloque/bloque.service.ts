...
export class BloqueService {
    constructor(
        ...
        // Iniciarlizar los bloques pred. si no existem
    ...
    async inicializarBloquesDefault(){
        ...
        if( count === 0){
            //crear bloques horarios de 1 hora desde 8 a 20 hrs
    ...
    async create(createBloqueDto: CreateBloqueDto): Promise<ApiResponse<Bloque>> {
        try{
            ...
            const saveBloque = await this.bloqueRepository.save(bloque);

            return CreateResponse<Bloque>(
                'Bloque horario creado exitosamente', saveBloque, 'CREATED'
            );
        }
        catch (error) {
            return CreateResponse<Bloque>(
                'Error al crear el bloque horario', null as unknown as Bloque,
                'BAD_REQUEST', error.message, false
            );
        }
    }
    async findAll(): Promise<ApiResponse<Bloque[]>> {
        try {
            ...
            return CreateResponse<Bloque[]>(
                'Bloques horarios obtenidos exitosamente', bloques, 'OK'
            );
        }
        catch (error) {
            return CreateResponse<Bloque[]>(
                'Error al obtener los bloques horarios',  null as unknown as Bloque[],
                'INTERNAL_SERVER_ERROR', error.message, false
            );
        }
    }
    ...
    async findOne(id: number): Promise<ApiResponse<Bloque>> {
        try {
            ...
            if(!bloque){
                throw new NotFoundException('No se encontro el bloque horario con ID ${id}');
            }

            return CreateResponse<Bloque>(
                'Bloque horario obtenido exitosamente', bloque, 'OK'
            );
        }
        catch(error){
            if (error instanceof NotFoundException){
                return CreateResponse<Bloque>(
                    'Bloque horario no encontrado', null as unknown as Bloque,
                    'NOT_FOUND', error.message, false
                );
            }

            return CreateResponse<Bloque>(
                'Error al obtener el bloque horario',
                null as unknown as Bloque, 'INTERNAL_SERVER_ERROR',
                error.message, false
            );
        }
    }
    ...
    async update(id: number, updateBloqueDto: UpdateBloqueDto): Promise<ApiResponse<Bloque>> {
        try{
            ...
            if(!bloque){ throw new NotFoundException('No se encontro el bloque horario con ID $[id');}
            await this.bloqueRepository.update(id, updateBloqueDto);
            const updateBloque = await this.bloqueRepository.findOne({ where: { id}});

            return CreateResponse<Bloque>(
                'Bloque horario actualizado exitosamente', updateBloque, 'OK'
            );
        }
        catch (error){
            if (error instanceof NotFoundException){
            ...
            return CreateResponse<Bloque>(
                'Error al actualizar el bloque horario', null as unknown as Bloque,
                'INTERNAL_SERVER_ERROR', error.message, false
            );
        }
    }
    ...
    async remove(id: number): Promise<ApiResponse<null>> {
        try{
            ...
            return CreateResponse<null>(
                'Bloque horario eliminado exitosamente', null, 'OK'
            );
        }
        catch(error){
            if(error instanceof NotFoundException){
                return CreateResponse<null>(
                    'Bloque horario no encontrado', null, 'NOT_FOUND',
                    error.message, false
                );
            }

            return CreateResponse<null>(
                'Error al eliminar el bloque horario', null, 'INTERNAL_SERVER_ERROR',
                error.message, false
            );
        }
    }
    ...
    async findActivos(): Promise<ApiResponse<Bloque[]>>  {
        try {
            const bloqueActivos = await this.bloqueRepository.find({
                where: {activo: true},
                order: { horaInicio: 'ASC'}
            });
            ...
            return CreateResponse<Bloque[]> (
                ...
                null as unknown as Bloque[], 'INTERNAL_SERVER_ERROR', error.message, false
            );
        }
    }
}